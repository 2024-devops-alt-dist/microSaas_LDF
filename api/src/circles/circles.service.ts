...
export class CirclesService {
    ...
    async findMyCircles(userId: string): Promise<Circle[]> {
        return this.circleModel.find({ 'members.userId': userId }).exec();
    }
    ...
    // User action methods
    async requestJoinCircle(circleId: string, memberData: CircleMember): Promise<Circle> {
        const exists = await this.circleModel.findOne({
            _id: circleId,
            $or: [
                { 'members.userId': memberData.userId },
                { 'requests.userId': memberData.userId }
            ]
        });
    ...
    async approveRequest(circleId: string, userId: string): Promise<Circle> {
        ...
        if (!circle) {
            throw new NotFoundException(`No pending request found for user ID ${userId} in circle ID ${circleId}`);
        }

        const memberData = circle.requests?.find(req => req.userId === userId);
        const updatedCircle = await this.circleModel.findByIdAndUpdate(
            circleId,
            {
